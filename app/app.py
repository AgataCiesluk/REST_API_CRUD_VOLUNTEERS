...
def update_volunteer(id):
    ...
    #     here should be a method volunteer = db_update_volunteer(id, db_model, request.args.get('first_name'), request.args.get('last_name'), request.args.get('phone_number'))
    except NoRecordFound:
        return jsonify({'error': f'Record with id={id} not found in DB'}), 404
    # first_name = request.args.get('first_name')
